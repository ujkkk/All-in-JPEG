...
import com.example.all_in_jpeg.AudioModule.AudioResolver
import com.example.all_in_jpeg.Contents.ContentAttribute
import com.example.all_in_jpeg.Contents.ContentType
import com.example.all_in_jpeg.Contents.Picture
import com.example.all_in_jpeg.SaveModule.SaveResolver
import kotlinx.coroutines.*


class AllInContainer(_activity: Activity) {
...
//    }
    ...
    fun save(isAllInJPEG : Boolean) : String {
        return saveResolver.save(isAllInJPEG)
    }
    fun overwiteSave(isAllInJPEG : Boolean, fileName : String) : String{
       return saveResolver.overwriteSave(isAllInJPEG, fileName)
    }
    // 사진을 찍은 후에 호출되는 함수로 MC Container를 초기화하고 찍은 사진 내용으로 MC Container를 채움
    suspend fun setImageContent(byteArrayList: ArrayList<ByteArray>, type: ContentType,
                                contentAttribute : ContentAttribute) : Boolean= withContext(Dispatchers.Default){
