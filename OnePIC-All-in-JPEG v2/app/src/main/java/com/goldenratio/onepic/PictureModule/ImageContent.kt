...
//        setBitmapList()
    ...
    /**
     * 기존 metadata의 APP1을 newApp1Data로 교체 후 변경 된 metadata 리턴
     */
        ...
        if(jpegMetaData != null) {
            ...
            if(findAPP1){
                ...
                byteBuffer.write(
                    ...
                    jpegMetaData.size - (app1StartPos + app1DataSize)
                )
    ...
    /**
     * 이미지 데이터 중 APP1 세그먼트를 찾고 해당 부분을 추출하여 리턴. 만약 썸네일이 존재하면 썸네일을 제외한 APP1 데이터 리턴
     */
...
//            return ByteArray(0)
    ...
    fun findSOFMarkers (jpegBytes: ByteArray) : ArrayList<Int> {
        ...
        while (SOFStartInex < jpegBytes.size/2 - 1) {
            ...
            SOFStartInex++
            // EOI 마커를 만나면 탐색 중단
            if(jpegBytes[SOFStartInex] == 0xFF.toByte() && jpegBytes[SOFStartInex + 1] == 0xD9.toByte()){
                countFindingEOI++
                if(countFindingEOI == 2)
                    break
            }
